...
// Email configuration - using multiple providers for reliability
const EMAIL_CONFIGS = {
  // Primary: Gmail with SSL
  gmail: {
    host: 'smtp.gmail.com',
    port: 465,
    secure: true,
...
// Try to create a working transporter
async function createTransporter() {
  for (const [name, config] of Object.entries(EMAIL_CONFIGS)) {
    try {
  ...
  // If all fail, create a dummy transporter for graceful degradation
  console.log('⚠️ All email configurations failed, using fallback mode');
  transporter = {
    sendMail: async (mailOptions) => {
      ...
      return {
        ...
        response: 'Email queued for manual sending'
      };
    }
  };
...
app.post('/api/send-email', async (req, res) => {
  ...
  try {
    ...
    console.log('📧 Using configuration:', currentConfig);
    
    res.json({
      success: true,
      message: currentConfig === 'fallback' ? 'Email queued for manual sending' : 'Email sent successfully',
